...
A `Maybe` is a built-in [custom type](/book/syntax/custom_types/) with two variants: `Just something` where `something` is the value you want, or `Nothing`.

This is a constant we're defining as a convenience to hold our server configuration:
...
A Result is a built-in [custom type](/book/syntax/custom_types/) with two variants:
...
We will respond to that message in our `update` function.
