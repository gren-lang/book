...
That means it cannot affect or be affected by "the outside world".
The only events are those explicitly triggered by the user.
In our case, clicking the plus or minus buttons.

We're defining the program with a record that points to our `init`, `update`, and `view` functions.
