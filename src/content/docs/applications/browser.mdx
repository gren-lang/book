...
One way of doing that is by sending [commands](https://packages.gren-lang.org/package/gren-lang/core/version/latest/module/Platform.Cmd).
...
Unlike a sandbox, elements have the ability to send commands and subscriptions to the [Gren Runtime](/book/applications/runtime/).
...
When the user clicks the button, `update` is called with a `ClickedButton` message, and we respond with a call to another function we wrote, `getRandomNumber`.
...
This may seem convoluted at first, but it's a way that Gren keeps your code easier to understand and maintain, since all side effects will go through your `update` function in a way that's deterministic and covers every edge case.
...
It takes a generator that says what type of random value we want, and a message to trigger with that random value.
...
It's one of the variants of our `Msg` [custom type that can hold an integer](/book/syntax/custom_types/#types-with-data).

TODO: diagram of init, update, subscriptions, and runtime?
