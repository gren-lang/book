...
We'll compile this program just like the others:

```sh
gren make src/Main.gren
```

But we also need to install the [url package](https://packages.gren-lang.org/package/gren-lang/url/version/latest/overview) so we can work with URLs:
...
The biggest difference between an applicatino and a document is an application will capture link clicks and URL changes and send us messages instead of reloading the page, so we can handle them however we want.
So we need to tell Gren what messages we expect:
...
We're saving it because Gren requires this key to call functions that change the URL.
...
```elm title="src/Main.gren" showLineNumbers startLineNumber=57 {"1":4} {"2":9} {"3":12} {"4":17}
update : Msg -> Model -> { model : Model, command : Cmd Msg }
update msg model =
    case msg of
        UrlRequested urlRequest ->
...
1. When someone clicks a link, we told Gren that we'd like to receive this `UrlRequested` message.
   It will [hold](/book/syntax/custom_types/#types-with-data) a [`Browser.UrlRequest`](https://packages.gren-lang.org/package/gren-lang/browser/version/latest/module/Browser#UrlRequest).
...
4. When the URL changes via `pushUrl`, we told Gren that we'd like to receive this `UrlChanged` message.
   All we do is update the page value on our model.
